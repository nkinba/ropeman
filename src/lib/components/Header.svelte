<script lang="ts">
	...
	let { onsettings, onnewproject, onconnect, onanalyze }: {
		onsettings?: () => void;
		onnewproject?: () => void;
		onconnect?: () => void;
		onanalyze?: () => void;
...
<header class="header">
	...
	<div class="header-right">
		...
		{/if}
		{#if hasProject}
			<button class="header-btn text-btn" onclick={() => onanalyze?.()} title="AI Semantic Analysis">
				<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
		...
		<button class="header-btn" onclick={toggleTheme} title={$t($theme === 'dark' ? 'lightMode' : 'darkMode')}>
			{$theme === 'dark' ? '\u2600' : '\u263D'}
		</button>
