...
- 새 세션 시작 시 순서대로 수행
  1. `/catchup` → docs/history 최신 3개 읽고 맥락 파악
...
- 스프린트 종료 시 PRD 로드맵 업데이트 규칙
  - PRD '7. 우선순위 및 로드맵'은 3개 섹션으로 관리한다:
    - **이전 스프린트**: 직전 스프린트 1개만 표시 (sprint_id + 완료/미완료 태스크). 이전 이력은 `docs/history/`에서 관리.
...
- 스프린트 진행 중 버그를 발견하고 해결할 때마다 `docs/bug-report/`에 기록한다.
- `docs/bug-report/INDEX.md` — 버그 유형별 인덱스. 새 유형 추가 시 여기도 업데이트.
...
- `/catchup` — docs/history 읽고 이전 작업 맥락 파악
